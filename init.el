...
(use-package exwm
  ...
  :demand t
  :preface
  (defun my/exwm-one-display ()
    (start-process-shell-command
     "one-display" nil
     "outs=$(xrandr | awk '$2==\"connected\"{print $1}')
      chosen=$(printf '%s\n' $outs | awk '!/^(eDP|LVDS)-/{print; exit}')
      [ -n \"$chosen\" ] || chosen=$(printf '%s\n' $outs | awk '/^(eDP|LVDS)-/{print; exit}')
      [ -n \"$chosen\" ] || exit 1
      set -- --output \"$chosen\" --auto --primary
      for o in $outs; do
        [ \"$o\" = \"$chosen\" ] || set -- \"$@\" --output \"$o\" --off
      done
      xrandr \"$@\""))
  (defun my/exwm-one-display-later ()
    (run-at-time 1 nil #'my/exwm-one-display))
  (defun my/exwm-run-once (command)
    (when (executable-find command)
      (unless (get-process command)
        (start-process-shell-command command nil command))))
  (defun my/exwm-start-applets ()
    (my/exwm-run-once "nm-applet")
    (my/exwm-run-once "pasystray"))
  (defun my/exwm-run-emacs-key (key)
    (interactive (list (read-key-sequence "Emacs key: ")))
    (let ((cmd (key-binding key t)))
      (unless cmd
        (user-error "No Emacs binding for %s" (key-description key)))
  ...
  (setq exwm-workspace-number 2
        ...
        `((,(kbd "s-SPC") . my/exwm-run-emacs-key)
          (,(kbd "s-s")   . my/exwm-next-workspace)
          (,(kbd "s-r")   . exwm-reset)))

  :config
  (add-hook 'exwm-init-hook #'my/exwm-set-keyboard)
  (add-hook 'exwm-init-hook #'my/exwm-one-display)
  (add-hook 'exwm-init-hook #'my/exwm-start-applets)
  (add-hook 'exwm-update-class-hook #'my/exwm-rename-buffer)

  (require 'exwm-randr)
  (add-hook 'exwm-randr-screen-change-hook #'my/exwm-one-display-later)
  (exwm-randr-mode 1)

  (require 'exwm-systemtray)
  (setq exwm-systemtray-position 'top
        exwm-systemtray-height 24
        exwm-systemtray-icon-gap 4
        exwm-systemtray-background-color
        (or (face-background 'mode-line nil t) "#222222"))
